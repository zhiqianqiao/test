...
class PercParser:
    def __init__(self, nav_map, p):
        ...
        self.cell_upper = np.ones(p.cell_num) * p.cell_border_max
        self.cell_lower = -np.ones(p.cell_num) * p.cell_border_max
        self.cell_upper[[1, 7]] = p.cell_border_01
        self.cell_upper[[2, 8]] = -p.cell_border_01
        self.cell_upper[4] = p.cell_border_34
        self.cell_upper[5] = -p.cell_border_34
        self.cell_lower[[0, 6]] = p.cell_border_01
        self.cell_lower[[1, 7]] = -p.cell_border_01
        self.cell_lower[3] = p.cell_border_34
        self.cell_lower[4] = -p.cell_border_34
    ...
    def parse(self, loc_hist, in_perc):
        ...
        for i in range(self.p.cell_num):
            self.cells[i] = self.nav_map.get_extension(loc_list[i], self.cell_upper[i], self.cell_lower[i])
